...
class Portfolio:
    ...
    def initialize_holdings(self, stock_price, bond_price):
        stock_value = self.initial_wealth * self.target_stock_weight
        bond_value = self.initial_wealth * self.target_bond_weight
        self.stock_shares = stock_value / stock_price
        self.bond_shares = bond_value / bond_price
    ...
    def rebalance(self, stock_price, bond_price, transaction_cost, date):
        ...
        current_stock_val = self.stock_shares * stock_price
        current_bond_val = self.bond_shares * bond_price
        
        target_stock_gross = total_value * self.target_stock_weight
        target_bond_gross = total_value * self.target_bond_weight
        
        turnover = abs(target_stock_gross - current_stock_val) + abs(target_bond_gross - current_bond_val)
        cost = turnover * tc
        
        net_wealth = total_value - cost
        self.stock_shares = (net_wealth * self.target_stock_weight) / stock_price
        self.bond_shares = (net_wealth * self.target_bond_weight) / bond_price
...
def simulate_ml_rebalancing(prices, ml_predictions, prob_threshold=None, transaction_cost=None, initial_wealth=None):
    iw = initial_wealth or config.initial_wealth
    tc = transaction_cost if transaction_cost is not None else config.transaction_cost[1]
    pt = prob_threshold if prob_threshold is not None else config.prob_threshold
    ...
    for i, (date, row) in enumerate(prices.iterrows()):
        ...
        prob = ml_predictions.get(date, 0.0)
        
        if pending is None and i % step == 0 and prob > pt and i < len(prices) - lag:
            pending = lag
...
def calculate_performance_metrics(results, initial_wealth=None):
    ...
    return {"total_return": total_ret, "annualized_return": ann_ret, "annualized_volatility": ann_vol, "sharpe_ratio": sharpe, "max_drawdown": dd.min(), "n_rebalances": results["rebalanced"].sum(), "avg_drift": results["drift"].mean(), "max_drift": results["drift"].max(), "final_value": v.iloc[-1], "total_costs": results["cost"].sum(), "total_turnover": results["turnover"].sum()}